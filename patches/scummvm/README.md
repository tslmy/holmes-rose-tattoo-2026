...
| `rosetattoo-hires-map-tooltip-scroll-fix.patch` | Follow-up to `rosetattoo-hires-tooltip-text-fix.patch`: fixes hires tooltip text on the overhead travel map leaving ghosted/phantom copies once the map view has scrolled away from `(0, 0)` (e.g. entering the map from a room, then hovering a distant location icon to trigger auto-scroll). `WidgetTooltipBase::_bounds`/`_oldBounds` are in world/scroll space, but `registerRoseTattooHiresTextRect()`/`clearRoseTattooHiresTextRect()` expect native screen-space rects; converts by subtracting `screen._currentScroll` before calling them. Depends on `rosetattoo-hires-font-ttf-override.patch` and `rosetattoo-hires-tooltip-text-fix.patch`. |
